...
name_array = cellstr(const.name);
for k=1:size(names,1),
  tmp = find(strcmp(deblank(names(k,1:4)),name_array));
%   tmp=strmatch(names(k,1:4),const.name);
  if isempty(tmp),
      disp(['Unexpected name encountered. Unable to make nodal corrections ',names(k,1:4)])
      ju = 0;
      break
  end
  ju(k)=tmp;
...
if ~isempty(lat) && abs(jdmid)>1 && max(ju)~=0,
  [v,u,f]=t_vuf(jdmid,ju,lat);
elseif abs(jdmid)>1 && max(ju)~=0, % a real date
