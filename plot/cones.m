...
%   fix     = all cones are of constant size fix
%
...
for aa = 1:numcones
    index = (aa-1)*vlen+1:aa*vlen;
    len = norm([x(aa),y(aa),z(aa)]) * scale;
    verts(index,3) = verts(index,3) * len;
    verts(index,1:2) = verts(index,1:2) * len*conewidth;
    
    verts(index,:) = coneorient(verts(index,:),  [x(aa),y(aa),z(aa)]);
    
    verts(index,1) = verts(index,1) + x_orig(aa);
    verts(index,2) = verts(index,2) + y_orig(aa);
    
    % -- maintain circularity of cones despite DataAspectRatio
    verts(index,3) = verts(index,3) * zscale;
    verts(index,3) = verts(index,3) + z_orig(aa);
end
