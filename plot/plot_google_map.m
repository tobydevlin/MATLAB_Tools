...
drawnow;
global inputParams
if isfield(inputParams,['ax' num2str(double(gca)*1e6,'%.0f')])
    params = inputParams.(['ax' num2str(double(gca)*1e6,'%.0f')]);
...
for idx = 1:length(axes_objs)
    if ~isempty(findobj(get(axes_objs(idx),'children'),'tag','gmap'));
        if isfield(inputParams,['ax' num2str(double(axes_objs(idx))*1e6,'%.0f')])
            params = inputParams.(['ax' num2str(double(axes_objs(idx))*1e6,'%.0f')]);
        else
            params = {};
        end
        axes(axes_objs(idx));
