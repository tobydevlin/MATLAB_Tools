...
classdef (CaseInsensitiveProperties = true) fvres_sheet < fvres
    ...
    methods
        ...
        function set.StampVec(obj,val)
            if ~islogical(val)
                error('expecting vector of logicals for StampVec')
            end
            if isfield(obj.WORK,'idx2') && length(val) ~= max(obj.WORK.idx2) % 1st logical incase StampVec is the 1st property to be set. This could be delaying a nasty error
                error('expecting vector of length NumCells2D for StampVec')
            end
            obj.StampVec = val;
            obj.WORK.refresh = true;
            obj.WORK_C2N.refresh = true;
