...
classdef fvg < hgsetget
    ...
    methods
        ...
        function obj = fvg(controlObj,resfil,fvresObj)
            ...
            set(obj.PatchObj,'ButtonDownFcn',@(src,evnt)set_gcfvg(obj,src,evnt))
        ...
        function set.Variables(obj,val)
            var = fv_variables(val); % _x will always preceed _y
            switch obj.Type
                ...
                case 'fvg_sheetvec'
                    if length(var) ~= 2
                        error('expecting 2 variables ( _x & _y) for fvg_sheetvec objects')
                    end
            end
            ...
            switch obj.Type
                ...
                case 'fvg_sheetvec'
                    if ~iscell(val) || length(val) < 2
                        error('expecting a cell array of length 2 for Expression property for fvg_sheetvec')
                    end
            end
...
function set_gcfvg(obj,~,~)
if isempty(whos('gcfvg','base'))
    evalin('base','global gcfvg;') % gcfvg may not exist in base workspace
end
global gcfvg
gcfvg = obj;
end
