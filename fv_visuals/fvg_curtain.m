...
%
%   PlineObj       ==> object created by hggroup.m which marks the position of the Pline
%
...
%
%   FaceColor      {flat} | ColorSpec | none ==> color of cell faces
%
%   EdgeColor      {none} | ColorSpec ==> color of cell edges
%
%
...
classdef (CaseInsensitiveProperties = true) fvg_curtain < fvg
    ...
    methods
        ...
        function obj = fvg_curtain(controlObj,resfil,geofil,pline,varargin)
            ...
            if isempty(pline)
                h_lin = myline;
                pline(:,1) = get(findobj(h_lin,'LineStyle','-'),'XData');
                pline(:,2) = get(findobj(h_lin,'LineStyle','-'),'YData');
            else
                h_lin = myline(pline(:,1),pline(:,2));
                set(h_lin,'Visible','off')
            end
        ...
        function set.Pline(obj,val)
            if isempty(val)
                ...
                obj.PlineObj = myline(val(:,1),val(:,2));
            end
            pline(:,1) = get(findobj(obj.PlineObj,'LineStyle','-'),'XData');
            pline(:,2) = get(findobj(obj.PlineObj,'LineStyle','-'),'YData');
