...
%
%   FaceColor      {flat} | interp | none | ColorSpec ==> color of cell faces
%
    ...
    methods
        ...
        function set.Vectors(obj,val)
            switch lower(val)
                ...
                otherwise
                    error('expecting input "on" or "off" for Vectors Property')
            end
            obj.Vectors = val;
        end
        ...
        function updatepatchdata(obj)
            if obj.ResObj.Feed
                if length(obj.Expression) == 2
                    vars = {'custom_x';'custom_y'};
                else
                    vars = {'custom'};
                end
            else
                vars = get(obj,'Variables');  % what about bed variables which are assigned
            end
            if ~isempty(vars)
