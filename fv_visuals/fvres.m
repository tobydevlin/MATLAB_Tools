        ...
        function obj = fvres(resfil)
            ...
            for aa = 1:nm
                if exist(resfil{aa},'file')
                    ...
                    obj.Nci(aa) = netcdf.open(resfil{aa},'NOWRITE');
                else
        ...
        function set.TimeCurrent(obj,val)
            for aa = 1:length(obj.Resfil)
                ...
                elseif val < obj.TimeVector.(m_name)(1)
                    ...
                    [~,i] = min(abs(obj.TimeVector.(m_name) - val));
%                     i = find(dt == min(dt),1,'first');
        ...
        function set.Variables(obj,val)
            ...
            [var_unlim,~] = netcdf_variables_unlimited(obj.Nci(1)); % the (1) is for if setting a variable after using an Expression with 2 or more files
